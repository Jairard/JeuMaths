...
func _process_node(dt : float, components : Dictionary) -> void:
	...
	if current_health <= 0:
			...
			if node_hero != null:
				...
				comp_health.get_node().get_parent().Fade.change_scene(FileBankUtils.loaded_scenes["death"])
			if node_enemy != null:			
				FileBankUtils.victories += 1
				FileBankUtils.scene_counter += 1
				comp_health.get_node().get_parent().Fade.change_scene(FileBankUtils.loaded_scenes["rewards"])
