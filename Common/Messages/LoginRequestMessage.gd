...
func _validate(args : Array) -> bool:
	return len(args) == 2 && args[argument.PlayerID] is String && args[argument.stats] is Array

# Here we store the argument value in the nickname variable
func _build(args: Array) -> void:
	PlayerID = args[argument.PlayerID] 		#args[int(argument.nickname)]
